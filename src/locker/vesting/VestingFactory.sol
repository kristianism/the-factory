 ...
 */
contract VestingFactory is Ownable2Step, Pausable, ReentrancyGuard, CollectorHelper, Referral {
    ...
    event LockerCreated(
        address indexed locker, address indexed creator, uint64 startTimestamp, uint64 durationSeconds, uint256 lockerId
    );
    ...
    constructor(
        ...
        if (_initialOwner == address(0) || _lockerImplementation == address(0) || _feeCollector == address(0)) revert ZeroAddress();

        if (_lockerImplementation.code.length == 0) revert InvalidImplementationAddress();
    ...
    function createLocker(
        ...
        Vesting(locker).initialize(msg.sender, _startTimestamp, _durationSeconds);
