 ...
 */
contract RegistrationAirdropFactory is Ownable2Step, Pausable, ReentrancyGuard, CollectorHelper, Referral {
    ...
    constructor(address _airdropImplementation, address _initialOwner, address _feeCollector, uint256 _creationFee)
        Ownable(_initialOwner)
        CollectorHelper(_feeCollector)
    {
        if (_initialOwner == address(0) || _airdropImplementation == address(0) || _feeCollector == address(0)) {
            revert ZeroAddress();
        }

        if (_airdropImplementation.code.length == 0) revert InvalidImplementationAddress();
    ...
    function createAirdrop(address _token, uint256 _baseAmount, address _referrer)
        external
        payable
        whenNotPaused
        nonReentrant
        returns (address airdrop)
    {
        ...
        RegistrationAirdrop(airdrop).initialize(_token, _baseAmount, msg.sender);

        IdToAddress[airdropCounter] = airdrop;
        creatorToAirdrop[msg.sender].push(airdrop);

        airdropInfo[airdrop] =
            AirdropInfo({airdropAddress: airdrop, creator: msg.sender, token: _token, airdropId: airdropCounter});
    ...
    function collectTokens(address airdrop) external onlyOwner nonReentrant {
    ...
    function isValidAirdrop(address airdrop) external view returns (bool) {
        return airdrop != address(0) && airdropInfo[airdrop].airdropAddress == airdrop;
    }
}
